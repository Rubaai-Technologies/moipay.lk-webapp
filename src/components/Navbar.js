      ...
      <div className="hidden md:flex items-center gap-11">
        {navLinks.map(link => (
          <Link
            key={link.name}
            to={link.path}
            className={`flex flex-col justify-center text-xl font-semibold transition-colors duration-200 font-['Wix_Madefor_Text',Inter,sans-serif] leading-8 ${
        ...
        <div className=" absolute top-20 right-4 bg-[rgba(35,39,47,0.98)] border border-cyan-400 rounded-lg shadow-lg flex flex-col w-32 z-50 animate-fade-in">
          {navLinks.map(link => (
            <Link
              key={link.name}
              to={link.path}
              className={`px-6 py-3 text-white font-semibold border-b border-gray-700 last:border-b-0 ${
                location.pathname === link.path 
